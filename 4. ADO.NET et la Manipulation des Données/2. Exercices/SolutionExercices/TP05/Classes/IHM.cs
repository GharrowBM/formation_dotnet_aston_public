...
namespace TP05.Classes
{
    internal class IHM
    {
        ...
        private void VoirEmprunts()
        {
            ...
            var emprunts = _context.Emprunts.Include(emprunt => emprunt.Client).Include(emprunt => emprunt.Films).ToList();

            emprunts.ForEach(emprunt =>
            {
                Console.WriteLine($"Emprunt du {emprunt.EmprunteLe.ToString("dddd dd MMMM yyyy")} par {emprunt.Client.Prenom} {emprunt.Client.Nom} {((DateTime.Now - emprunt.EmprunteLe).Days > 14 ? "EN RETARD" : "")}");
            ...
            else
            {
                ...
                else
                {
                    ...
                    else
                    {
                        ...
                        {
                            Console.WriteLine($"L'emprunt a été ajouté avec succès ! Date de retour avant frais de retard : {nouvelEmprunt.EmprunteLe.AddDays(14).ToString("dddd dd MMMM yyyy")}");
                        }
                        else
                        {
        ...
        private void ModifierFilm()
        {
            ...
            else
            {
                ...
                _context.Films.Update(filmTrouve);
                if (_context.SaveChanges() > 0)
                {
